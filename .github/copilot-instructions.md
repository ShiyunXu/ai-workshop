...
| 09 - Adding AI to an Existing App | `eShopLite-start/`, `eShopLite/`, and `StoreApp/` | Capstone. `eShopLite-start/` is the AI-free starting point, `eShopLite/` is the completed Aspire-based answer key, and `StoreApp/` is the consolidated completed snapshot. |
...
- Console samples (Parts 2, 3, 8) read credentials from **user secrets**: `AzureOpenAI:Endpoint` and `AzureOpenAI:Key`. Part 9 uses the `eShopLite-start/` and `eShopLite/` projects plus the `StoreApp/` project for its Azure OpenAI user secrets and optional `LocalModel:Endpoint` / `LocalModel:Model` settings for its Foundry Local step. Part 11 uses the Aspire connection string `ConnectionStrings:openai`.
