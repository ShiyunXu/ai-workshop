...
| 9 - Adding AI to an Existing App | Run the completed `StoreApp` snapshot and also verify the `eShopLite-start/` and `eShopLite/` snapshots if the README references them. The local-inference module needs `LocalModel:Endpoint` / `LocalModel:Model` (Ollama or Foundry Local) — note it as skipped if unavailable | `Part 09 - Adding AI to an Existing App/eShopLite-start/`, `Part 09 - Adding AI to an Existing App/eShopLite/`, `Part 09 - Adding AI to an Existing App/StoreApp/` |
