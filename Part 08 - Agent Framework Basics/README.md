...
>
> **Prerequisites**: Complete [Part 2: Build Chat App](../Part%2002%20-%20Build%20Chat%20App/README.md) and [Part 3: Add RAG](../Part%2003%20-%20Add%20RAG/README.md). [Part 5: MCP Server Basics](../Part%2005%20-%20MCP%20Server%20Basics/README.md) is recommended (the MCP section references it) but not required.
...
That mental model is what Part 9 needs, where the job is adding AI to an *existing* application rather than building a new one.
...
| --- | --- | --- |
| **Model** | The reasoning engine | The `IChatClient` from Parts 2-4 |
| **Instructions** | The system prompt that defines role and boundaries | The system message in your chat loop |
| **Tools** | Functions the model may call to act or fetch data | Function calling; the MCP tool you built in Part 5 |
...
[Microsoft Agent Framework (MAF)](https://learn.microsoft.com/agent-framework/overview/agent-framework-overview) adds one primary abstraction on top of what you already have: `AIAgent`. For example, your provider setup from Part 2 works exactly as it is; you just add one package:
...
Part 5 built an MCP server (and the optional Parts 6-7 go further). Their relationship to agents is simple:
...
- ✅ `AIAgent` wraps the same `IChatClient` you configured in Parts 2-4 and owns the tool-calling loop
...
📖 **Return to**: [Workshop Overview](../README.md) | 🔄 **Previous**: [Part 5: MCP Server Basics](../Part%2005%20-%20MCP%20Server%20Basics/README.md) | ➡️ **Next**: [Part 9: Adding AI to an Existing App](../Part%2009%20-%20Adding%20AI%20to%20an%20Existing%20App/README.md)
